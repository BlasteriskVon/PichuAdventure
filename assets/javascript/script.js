...
function test(x){
        ...
        option.onclick = "";
        if(x != "ducky"){
            option2.onclick = function() { test("ducky")};
        } else {
            option2.onclick = "";
        }
    }
...
function skip(id, text, finishingFunction){
    clearInterval(textInterval);
    var spot = $("#" + id);
    spot.text(text);
    finishingFunction();
}
function skipClass(class_name, text, finishingFunction){
    clearInterval(textInterval);
    var spot = $("." + class_name);
    spot.text(text);
    finishingFunction();
...
function rollingText(id, text, finishingFunction){
    ...
    function tempSkip(){
        ...
        skip(id, text, finishingFunction);
    }
...
function rollingTextClass(class_name, text, finishingFunction){
    ...
    function tempSkip(){
        spot.off("click", tempSkip);
        skipClass(class_name, text, finishingFunction);
    }
    spot.on("click", tempSkip);
    textInterval = setInterval(function() {
        if(textIndex < script.length){
            ...
            spot.off("click", tempSkip);
            skip(class_name, text, finishingFunction);
        }
    }, 50);
}
...
if(!mobileUsage){
rollingText("welcomeText", "Welcome to Pichu's Adventure!", function() {
    var optionOne = $("<div id=\"option1\" class=\"col-md-3\" style=\"font-size: 2em; color:white\"></div>");
    var optionTwo = $("<div id=\"option2\" class=\"col-md-3 options\" style=\"font-size: 2em; color:white\">Thanks!</div>");
    var optionThree = $("<div id=\"option3\" class=\"col-md-3 options\" style=\"font-size: 2em; color:white\">Ok</div>");
    var optionFour = $("<div id=\"option4\" class=\"col-md-3\" style=\"font-size: 2em; color:white\"></div>");
    var arrayToAdd = [optionOne, optionTwo, optionThree, optionFour];
    for(var i = 0;i < arrayToAdd.length;i++){
        $("#welcomeRow").append(arrayToAdd[i]);
    }
    var option1 = document.getElementById("option1");
    var option2 = document.getElementById("option2");
    var option3 = document.getElementById("option3");
    var option4 = document.getElementById("option4");
    var clickedNotPichu = false;
    option2.onclick = option3.onclick = function() {
        option2.onclick = option3.onclick = option2.innerText = option3.innerText = "";
        option2.className = option3.className = "col-md-3";
        rollingText("welcomeText", "Before we start, what mode would you like to play?", function() {
            option2.className = "col-md-3 options";
            option3.className = option4.className = "col-md-3";
            option1.className = "col-md-3 options alwaysHighlighted";
            option1.innerText = "Rush Mode";
            option2.innerText = "Story Mode (under construction)";
            option3.innerText = "";
            option4.innerText = "";
            option3.onclick = option4.onclick = ""
            option1.onmouseover = function() {
                option1.className = "col-md-3 options";
            }
            option2.onclick =  function() {
                clickedNotPichu = true;
                var tempFn = option1.onclick;
                var tempFn2 = option2.onclick;
                option1.onclick = "";
                option2.onclick = option3.onclick = option4.onclick = "";
                rollingText("welcomeText", "This mode is not available at the moment.", function() {
                    option1.onclick = tempFn;
                    option2.onclick = tempFn2;
                });
            }
            option1.onclick = function() {
                option1.onmouseover = "";
                option1.innerText = option1.onclick = option2.innerText = option2.onclick = option3.innerText = option3.onclick = option4.innerText = option4.onclick = "";
                option1.className = option2.className = option3.className = option4.className = "col-md-3";
                $("#player-pic").attr("src", pichuLoad.picture);
                rollingText("welcomeText", "Awesome! Rush Mode it is! Well, are you ready to begin?", function() {
                    option2.className = option3.className = "col-md-3 options";
                    option2.innerText = "Yes.";
                    option3.innerText = "Sure.";
                    option2.onclick = option3.onclick = function() {
                        option2.className = option3.className = option4.className = "col-md-3";
                        option2.innerText = option3.innerText = option4.innerText = "";
                        rollingText("welcomeText", "Okay then, start!", function() {
                            startMeBaby = true;
                    ...
                    if(clickedNotPichu){
                        option4.className = "col-md-3 options";
                        option4.innerText = "Why isn't Story Mode finished yet?";
...
if(event.key === "Enter"){
pichu.myArray = pichu.rightIdleArrays;
}
});
